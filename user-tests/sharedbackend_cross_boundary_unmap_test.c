// sharedbackend_cross_boundary_unmap_test.c
//
// Functional test for SharedBackend PMD split across a 2MiB boundary.
//
// Goal:
//   - Create a MAP_SHARED|MAP_ANONYMOUS mapping covering 4MiB (two 2MiB windows).
//   - Collapse both windows via madvise(MADV_COLLAPSE).
//   - munmap() a 16KiB range straddling the 2MiB boundary to force PMD split.
//   - Verify neighbor pages outside the hole keep their pattern and that reading
//     from the hole faults.
//
// Build for RISC-V:
//   riscv64-linux-gnu-gcc -O2 -static sharedbackend_cross_boundary_unmap_test.c -o sharedbackend_cross_boundary_unmap_test
//
#define _GNU_SOURCE
#include <errno.h>
#include <setjmp.h>
...
int main(void) {
    ...
    printf("=== sharedbackend_cross_boundary_unmap_test ===\n");

    uint8_t *p = mmap(NULL, LEN_4M, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    ...
    if (madvise(p, LEN_4M, MADV_COLLAPSE) != 0) {
        if (errno == ENOMEM) {
            printf("sharedbackend_cross_boundary_unmap_test: SKIP (ENOMEM on collapse)\n");
            munmap(p, LEN_4M);
            return 77;
        }
